...
onMounted(() => {
  const tl = gsap.timeline({
    scrollTrigger: {
      ...
      start: 'top top',
      end: '+=150%', // Short scroll distance for a punchy feel
      pin: true,
  ...
  .to(textRef.value, {
    ...
    textShadow: '-5px 0px 0px #00ff00, 5px 0px 0px #ff00ff',
    duration: 0.2,
    ease: 'steps(5)', // Jerky movement
  }, '<0.1');

  // 2. Scanlines Intensify (0 -> 0.3)
  tl.to(scanlinesRef.value, {
    opacity: 1,
    backgroundSize: '100% 2px', // Tighter lines
    duration: 0.3,
    ease: 'none',
  }, '<');

  // 3. Green Flash (0.3 -> 0.4) - The "Wipe"
  ...
  // 4. Clear Terminal / Switch Background (at flash peak)
  tl.set(portalRef.value, { backgroundColor: 'transparent' }, '>');
  tl.set(textRef.value, { opacity: 0 }, '<'); // Hide text
  tl.set(scanlinesRef.value, { opacity: 0 }, '<'); // Hide scanlines

  // 5. Flash Fade Out (0.4 -> 0.6)
  tl.to(flashRef.value, {
    opacity: 0,
    duration: 0.2,
    ease: 'power4.out',
...
<template>
  <section ref="portalRef" class="w-full! max-w-none! h-screen flex items-center justify-center bg-black relative overflow-hidden">
    ...
    <div ref="gridRef" class="absolute inset-0 bg-[linear-gradient(to_right,#a855f720_1px,transparent_1px),linear-gradient(to_bottom,#a855f720_1px,transparent_1px)] bg-[size:40px_40px] opacity-0 scale-150 pointer-events-none z-20"></div>
